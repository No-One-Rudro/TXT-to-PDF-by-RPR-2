...
const RenderCodesView: React.FC<Props> = ({ onBack, selectedEngine, onSelectEngine, t }) => {
  ...
  return (
    <div className="max-w-6xl mx-auto w-full p-8 flex-1 flex flex-col min-h-screen animate-in fade-in duration-500">
      <header className="py-10 flex items-center">
        ...
        <div className="ml-8">
          <h1 className="text-4xl font-black italic uppercase tracking-tighter leading-none">Render Codes</h1>
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-green-accent/60 mt-2">Active Logic Modules</p>
        </div>
      </header>
      ...
      <div className="flex-1 flex flex-col justify-center pb-20 overflow-hidden">
          ...
          {ENGINE_REGISTRY.map((eng) => (
            ...
            >
              ...
              <div className="p-8 text-center bg-black/20 rounded-b-[4rem]">
                 <p className="text-[9px] font-black uppercase tracking-[0.4em] opacity-20">Source: Engine{eng.label}.ts</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center space-x-2 mt-8">
           {ENGINE_REGISTRY.map((eng) => (
             <div key={eng.id} className={`h-1.5 rounded-full transition-all duration-500 ${selectedEngine === eng.id ? 'w-12 bg-green-accent' : 'w-3 bg-white/10'}`} />
           ))}
        </div>
      </div>
    </div>
...
export default RenderCodesView;
